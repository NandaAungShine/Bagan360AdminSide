...
function CarsOrder() {
  ...
  // ===== 3. API CONFIG =====
  const API_BOOKING_BASE = 'http://130.94.21.185:8000/api/admin/car/booking';
    ...
    try {
      ...
      if (result.success && Array.isArray(result.booking)) {
        const mapped = result.booking.map((item) => ({
          ...
          note: item.note || '',
          car_name: item.car_name || item.name || 'Car',
          car_type: item.car_type || 'N/A',
          driver_name: item.driver_name || 'N/A',
          license_plate: item.license_plate || 'N/A',
        ...
        <div className="modal-content" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              ...
              <div><strong>Type:</strong> {booking.car_type}</div>
              <div><strong>Driver:</strong> {booking.driver_name}</div>
              <div><strong>License Plate:</strong> {booking.license_plate}</div>
  ...
  const BookingCard = ({ booking }) => (
    <div className="hotel-card-vertical" style={{ cursor: 'default' }}>
      ...
      <div className="hotel-card-info">
        <h3 className="hotel-name">{booking.car_name}</h3>
        ...
        <div style={{ fontSize: '13px', color: '#666', marginBottom: '4px' }}>
          <span><i className="bi bi-tag"></i> {booking.car_type}</span>
  ...
  if (loading && bookings.length === 0) {
    return (
      <div className={`dashboard-container ${isDarkMode ? 'dark-theme' : 'light-theme'}`}>
        <Header title="Car Bookings" onThemeChange={handleThemeChange} />
  ...
  const summaryData = [
    ...
    { label: 'Cars', count: bookings.length, icon: 'bi-car-front-fill', color: '#6f42c1' },
  ];

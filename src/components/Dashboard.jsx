...
function Dashboard() {
  ...
  // ----- Existing Data (Daily, Weekly, Monthly, Yearly) -----
  const dailyData = [
  ...
  // ----- UPDATED Important Metrics (Order: Total Bookings, Total Revenue, Active Users, Active Agency/Shop, Today Active, Avg Rating) -----
  ...
  return (
    <div className={`dashboard-container ${isDarkMode ? 'dark-theme' : 'light-theme'}`}>
      ...
      <div className="time-range-selector">
        <button className={`time-btn ${timeRange === 'daily' ? 'active' : ''}`} onClick={() => setTimeRange('daily')}>
          <i className="bi bi-calendar-day"></i> Daily
        </button>
        <button className={`time-btn ${timeRange === 'weekly' ? 'active' : ''}`} onClick={() => setTimeRange('weekly')}>
          <i className="bi bi-calendar-week"></i> Weekly
        </button>
        <button className={`time-btn ${timeRange === 'monthly' ? 'active' : ''}`} onClick={() => setTimeRange('monthly')}>
          <i className="bi bi-calendar-month"></i> Monthly
        </button>
        <button className={`time-btn ${timeRange === 'yearly' ? 'active' : ''}`} onClick={() => setTimeRange('yearly')}>
          <i className="bi bi-calendar-year"></i> Yearly
        </button>
      </div>

      {/* Important Metrics - 6 Boxes in One Row (Order updated) */}
      ...
      {/* Second Row - 2 Charts (Existing) */}
      <div className="charts-grid-two">
        {/* Today's Active Users */}
        <div className="chart-card">
          <div className="chart-header">
            <h3><i className="bi bi-person-activity"></i> Today's Active Users</h3>
          </div>
          <div className="active-users-list">
            {todayActiveUsers.map((user, index) => (
              <div key={index} className="active-user-item">
                ...
                <div className="active-user-action">
                  <span className="action-badge">{user.action}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ============================================================
          NEW ROW 1: Booking Status + Recent Bookings
          ============================================================ */}
      <div className="charts-grid-two" style={{ marginBottom: '28px' }}>
        {/* Booking Status Distribution */}
        <div className="chart-card">
          <div className="chart-header">
            <h3><i className="bi bi-pie-chart"></i> Booking Status</h3>
          </div>
          <div className="status-chart-wrapper">
            <div className="status-donut-container">
              ...
              <div className="status-center-label">
                314 <small>total</small>
              </div>
            </div>
        ...
        <div className="chart-card">
          <div className="chart-header">
            <h3><i className="bi bi-clock-history"></i> Recent Bookings</h3>
            <a href="#" className="view-all">View All</a>
          </div>
          <div className="recent-bookings-table-wrap">
            <table className="recent-bookings-table">
              ...
              <tbody>
                <tr><td>Emily Chen</td><td>Hotel</td><td><span className="booking-status-badge confirmed">Confirmed</span></td><td style={{ textAlign: 'right' }} className="booking-amount">$320</td></tr>
                <tr><td>Robert Kim</td><td>Tour</td><td><span className="booking-status-badge pending">Pending</span></td><td style={{ textAlign: 'right' }} className="booking-amount">$180</td></tr>
                <tr><td>Lisa Park</td><td>Restaurant</td><td><span className="booking-status-badge completed">Completed</span></td><td style={{ textAlign: 'right' }} className="booking-amount">$95</td></tr>
                <tr><td>James Wong</td><td>Transport</td><td><span className="booking-status-badge cancelled">Cancelled</span></td><td style={{ textAlign: 'right' }} className="booking-amount">$210</td></tr>
                <tr><td>Maria Garcia</td><td>Hotel</td><td><span className="booking-status-badge confirmed">Confirmed</span></td><td style={{ textAlign: 'right' }} className="booking-amount">$450</td></tr>
              </tbody>
            </table>
          </div>
        </div>
      ...
      {/* ============================================================
          NEW ROW 2: Top Agencies + Quick Insights
          ============================================================ */}
      <div className="charts-grid-two">
        {/* Top Performing Agencies */}
        <div className="chart-card">
          <div className="chart-header">
            <h3><i className="bi bi-trophy"></i> Top Agencies</h3>
            <a href="#" className="view-all">View All</a>
          </div>
        ...
        <div className="chart-card">
          <div className="chart-header">
            <h3><i className="bi bi-lightning-fill"></i> Quick Insights</h3>
          </div>

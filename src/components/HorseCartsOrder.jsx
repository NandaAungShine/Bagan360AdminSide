...
function HorseCartsOrder() {
  ...
  const API_BOOKING_BASE = 'http://130.94.21.185:8000/api/admin/horse-cart/booking';
    ...
    try {
      ...
      if (result.success && Array.isArray(result.booking)) {
        const mapped = result.booking.map((item) => ({
          ...
          horsecart_id: item.horsecart_id,
          ...
          note: item.note || '',
          horsecart_name: item.horsecart_name || item.name || 'Horse Cart',
          cart_type: item.cart_type || 'N/A',
          driver_name: item.driver_name || 'N/A',
          horse_name: item.horse_name || 'N/A',
        ...
        <div className="modal-content" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div><strong>Horse Cart:</strong> {booking.horsecart_name}</div>
              <div><strong>Cart Type:</strong> {booking.cart_type}</div>
              <div><strong>Driver:</strong> {booking.driver_name}</div>
              <div><strong>Horse:</strong> {booking.horse_name}</div>
  ...
  const BookingCard = ({ booking }) => (
    <div className="hotel-card-vertical" style={{ cursor: 'default' }}>
      ...
      <div className="hotel-card-info">
        <h3 className="hotel-name">{booking.horsecart_name}</h3>
        ...
        <div style={{ fontSize: '13px', color: '#666', marginBottom: '4px' }}>
          <span><i className="bi bi-tag"></i> {booking.cart_type}</span>
          <span style={{ marginLeft: '8px' }}><i className="bi bi-person-badge"></i> {booking.driver_name}</span>
        </div>
  ...
  if (loading && bookings.length === 0) {
    return (
      <div className={`dashboard-container ${isDarkMode ? 'dark-theme' : 'light-theme'}`}>
        <Header title="Horse Cart Bookings" onThemeChange={handleThemeChange} />
  ...
  const summaryData = [
    ...
    { label: 'Carts', count: bookings.length, icon: 'bi-cart2', color: '#6f42c1' },
  ];
    ...
    <div className={`dashboard-container ${isDarkMode ? 'dark-theme' : 'light-theme'}`}>
      ...
      <div className="search-actions-row" style={{ marginBottom: '20px' }}>
        <div className="search-bar-wrapper">
          ...
          <input
            type="text"
            placeholder="Search by customer, cart..."
...
export default HorseCartsOrder;